...
function createTable() { // Function for Create Table in SQLite.
    ...
    db.transaction(function(tx) { tx.executeSql(createStatement, [], onSuccess, onError); });
}
...
function loadRecord(i) {// Function for display records which are retrived from database.
    ...
    $("#wateres").val((item['wateres']).toString()).change();
    $("#top").val((item['top']).toString()).change();
    $("#pump").val((item['pump']).toString()).change();
    $("#pump_manual").val((item['pump_manual']).toString()).change();
    $("#other_pump").val((item['other_pump']).toString());
    $("#operator").val((item['operator']).toString());
    $("#operator_type").val((item['operator_type']).toString()).change();
    $("#condition").val((item['condition']).toString()).change();
    $("#disused").val((item['disused']).toString()).change();
    $("#abandoned").val((item['abandoned']).toString()).change();
    $("#access").val((item['access']).toString()).change();
    $("#water_presence").val((item['water_presence']).toString()).change();
    $("#drinking_water").val((item['drinking_water']).toString()).change();
    $("#fee").val((item['fee']).toString()).change();
    $("#fixme").val((item['fixme']).toString()).change();
...
function showOne(id) {// Function For Retrive data from Database Display Selected record
    ...
    $("#btns_3").html('<button id="btnShow" type=submit class="ui-btn ui-shadow ui-corner-all" onclick="loadAndReset()">SHOW ALL</button>');
}
