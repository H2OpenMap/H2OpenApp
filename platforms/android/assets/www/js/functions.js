...
function checkSource() {
    if (source == '') {
        alert("Hey! no source defined, please log in!");
...
function validateForm() {
    ...
    if (myForm.photo.value == "") {
        alert("Error: Photo is empty!");
        myForm.takePhoto.focus();
        return false;
    }
        ...
        tx.executeSql(selectAllStatement, [], function(tx, result) {
            ...
            if (dataset.length != 0) {
                ...
                for (var i = 0, item = null; i < dataset.length; i++) {
                    ...
                    exif = exif.replace(/,/g, '.');
                    var photo = item['photo'].substr(item['photo'].lastIndexOf('/') + 1);
                    content += item['id'] + ',' + item['wateres'] + ',' + item['source'] + ',' + item['s_date'] + ',' + item['lat'] + ',' + item['lon'] + ',' + item['acc'] + ',' + item['top'] + ',' + item['pump'] + ',' + item['pump_manual'] + ',' + item['other_pump'] + ',' + item['operator'] + ',' + item['operator_type'] + ',' + item['condition'] + ',' + item['disused'] + ',' + item['abandoned'] + ',' + item['access'] + ',' + item['water_presence'] + ',' + item['drinking_water'] + ',' + item['fee'] + ',' + item['fixme'] + ',' + item['note'] + ',' + photo + ',' + exif + '\n';
...
function uploadFile(fileName, photoList) {
    ...
    function win(r) {
        ...
        uploadPhoto(fileName, photoList);
    }
...
//------------------------------------UPLOAD-PHOTO---------------------------------------->>
function uploadPhoto(fileName, photoList) {
