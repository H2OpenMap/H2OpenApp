...
var selectOneStatement = "SELECT * FROM Contacts WHERE id=?";
        ...
        tx.executeSql(selectAllStatement, [], function(tx, result) {
            ...
            if (dataset.length!=0){
	            for (var i = 0, item = null; i < dataset.length; i++) {
	                item = dataset.item(i);
	                var content = '<li>' + item['username'] + ' , ' + item['useremail'] + '    ' + '<a href="#" onclick="loadRecord(' + i + ');">edit</a>' + '    ' +
	                    '<a href="#" onclick="showOne(' + item['id'] + ');">show</a></li>';
